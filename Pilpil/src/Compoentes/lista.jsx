export default function Lista({titulo, props}){
    return(
            <div className="flex flex-col items-center justify-start w-full">
               <p className="w-35 h-10 md:w-100 md:h-15 text_sm md:text-[30px] text-center bg-orange-700 rounded-lg border-3 border-amber-950 ">~~{titulo}~~</p>
                <ul className=" flex flex-wrap items-center justify-center mt-5 gap-10 ml-35">
                                {props.map((plato)=>(
                                    <div className="w-60 h-110 flex flex-col items-center justify-between gap-2 bg-orange-200 text-black rounded-lg border-solid border-2 border-yellow-900" key={plato.id}>
